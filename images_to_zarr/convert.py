...
def convert_from_memory(
    ...
    *,
    ...
    """
    Convenience function to convert numpy arrays directly to Zarr format.

    This is a wrapper around the main convert() function that handles
    memory-to-zarr conversion without needing to specify all parameters.

    Parameters
    ----------
    images
        Numpy array of images in any format. Will be converted to NCHW format.
    output_dir
        Destination path for the zarr store.
    image_metadata
        Optional list of metadata dictionaries, one per image.
        If not provided, basic metadata will be generated.
    chunk_shape
        Chunk layout for the zarr array.
    compressor
    ...
    return convert(
        ...
        folders=None,  # No folders for memory conversion
        metadata=None,  # No CSV metadata file
        recursive=False,
        num_parallel_workers=1,  # No parallelism needed for memory conversion
        fits_extension=None,
        images=images,
